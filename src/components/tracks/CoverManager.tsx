...
export default function CoverManager({ entityType, entityId, onClose, onUpdated }: CoverManagerProps) {
  ...
  async function handleDelete(coverId: string) {
    ...
    if (res.ok) {
      setCovers((prev) => {
        ...
        // The main flag may have shifted server-side, refetch
        void fetchCovers();
        return next;
      });
      onUpdated?.();
  ...
  const canAddMore = covers.length < 5;
