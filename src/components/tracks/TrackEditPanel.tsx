...
export default function TrackEditPanel({ track, onClose, onSaved, knownArtistNames = [], knownComposerNames = [], knownWriterNames = [], inline = false }: TrackEditPanelProps) {
  ...
  const [isCollaboration, setIsCollaboration] = useState(track.isCollaboration ?? false);
    ...
    return (
      <div className="mt-1 rounded-xl border border-white/10 bg-[#0d0e15] shadow-lg overflow-hidden">
        ...
        <div className="px-4 py-3 space-y-3 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between rounded-lg border border-white/10 bg-white/4 px-3 py-2">
            <p className="text-xs font-medium text-white">This is a collaboration</p>
            <button
              type="button"
              role="switch"
              aria-checked={isCollaboration}
              onClick={() => setIsCollaboration((v) => !v)}
              className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${isCollaboration ? "bg-primary-500" : "bg-white/15"}`}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${isCollaboration ? "translate-x-4" : "translate-x-0"}`} />
            </button>
          </div>
          {/* Prompt */}
          <div className="space-y-1">
