...
export async function GET(
  ...
  const [cover] = await db
    .select()
    .from(coverImages)
    .where(and(eq(coverImages.id, coverId), eq(coverImages.entityType, "release"), eq(coverImages.entityId, id)));
  ...
  try {
    ...
    console.error(`[release-cover-image] failed for ${s3Key}:`, error);
    return NextResponse.json({ error: "Cover not found" }, { status: 404 });
  }
}
