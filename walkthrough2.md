...
## 2026-06-07 zo 07:44 (Frontend runtime-guard voor workspace-default persistence)

- Findings: Ondanks server-side guards kon de frontend store nog steeds `workspace-default` als raw ID naar `PATCH /api/tracks/[id]` sturen, wat onnodige sentinel-traffic geeft en in edge-cases tot inconsistent workspace gedrag kan leiden.
- Conclusions: Voeg een runtime normalisatie in de store toe: alleen UUID-ids naar de API sturen, en bij `workspace-default` eerst resolveren naar de echte default workspace UUID (of `null` fallback als die nog niet beschikbaar is).
- Actions: `src/lib/store.ts` aangepast met `toPersistedWorkspaceId(...)` en UUID-regex guard; `persistTrackWorkspaceAssignment(...)` accepteert nu `string | null`; `moveTrackToWorkspace` en `moveTracksToWorkspace` normaliseren voortaan eerst de workspace-id voordat de PATCH-call wordt gedaan; `src/components/Sidebar.tsx` buildVersion bijgewerkt naar `202606070744`; `melodiq-user.md` versie bijgewerkt naar `202606070744`; gevalideerd met `npm run build` (geslaagd, met bestaande Turbopack NFT warning), validated.

## 2026-06-07 zo 13:33 (Upload dedupe robuuster met audio-only hash)

- Findings: Duplicate-detectie bij upload was gebaseerd op de volledige bestandsbytes; hierdoor konden identieke audiotracks met andere container-metadata (zoals ID3-tags) als verschillende uploads doorlopen.
- Conclusions: Voor robuuste dedupe moet hashing alleen op de audiocontent gebeuren (metadata negeren), met een DB-level unique guard als extra bescherming tegen race-conditions bij gelijktijdige uploads.
- Actions: `src/app/api/tracks/route.ts` uitgebreid met audio-only hashing helpers (`stripMp3Metadata`, `stripWavMetadata`, `computeUploadAudioHash`) zodat sidecar/metadata-verschillen niet meewegen; duplicate error-handling uitgebreid met unieke-constraint detectie (`23505`) en nette `Duplicate upload detected.` afwijzing; `src/db/schema.ts` uitgebreid met `tracks_user_provider_audio_id_unique`; `src/db/init.ts` uitgebreid zodat deze index op nieuwe en bestaande databases wordt aangemaakt; nieuwe migratie toegevoegd `drizzle/0003_robust_upload_audio_dedupe.sql` en journal bijgewerkt; `src/components/Sidebar.tsx` buildVersion bijgewerkt naar `202606071333`; `melodiq-user.md` versie en upload-dedupe beschrijving bijgewerkt; validated.

## 2026-06-07 zo 14:12 (Track drag-and-drop preciezer boven/onder)
...
## 2026-06-07 zo 21:55 (Playlists persistent in database + remove from playlist)

- Findings: Playlist-data zat client-side in localStorage, waardoor playlists en trackvolgorde niet betrouwbaar account/device-overstijgend waren. Daarnaast ontbrak een directe trackactie om een song uit een playlist te verwijderen.
- Conclusions: Maak playlists server-authoritative (database + API) met expliciete volgorde per playlist, hydrateer playlists in views die ze gebruiken, en voeg in trackacties een remove-optie toe voor playlists waarin de track voorkomt.
- Actions: Nieuwe playlist API toegevoegd (`src/app/api/playlists/route.ts`, `src/app/api/playlists/[id]/route.ts`) met create/list/delete/add/remove/reorder acties; playlist helper toegevoegd (`src/lib/playlists.ts`); Drizzle migratie toegevoegd `drizzle/0004_persistent_playlists.sql` en journal bijgewerkt; playlist-store in `src/lib/store.ts` uitgebreid met server-hydratie (`loadPlaylists`, `hydratePlaylistsFromServer`) en persist-calls naar playlist API; `src/app/page.tsx`, `src/app/library/page.tsx`, `src/app/workspaces/[workspaceId]/page.tsx` laden playlists nu vanuit backend; `src/components/TrackList.tsx` ondersteunt `onManualOrderChange` callback zodat playlist reorder server-side kan worden opgeslagen; `src/components/tracks/TrackActionMenu.tsx` en `src/components/tracks/TrackCard.tsx` uitgebreid met `Remove from playlist`; `src/components/tracks/types.ts` playlist type uitgebreid; `src/components/Sidebar.tsx` buildVersion bijgewerkt naar `202606072155`; `melodiq-user.md` versie + playlistgedrag bijgewerkt; gevalideerd met `npm run build` (geslaagd, met bestaande Turbopack NFT warning), validated.
